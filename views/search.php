...
if ($_SESSION['page-url'] == "galery") {
  if (isset($_GET['key']) && $_GET['key'] != "") {
    ...
    foreach ($keys as $no => $data) {
      $data = strtolower($data);
      $quer .= "tbl_wisata.judul LIKE '%$data%'";
      if ($no + 1 < count($keys)) {
        $quer .= " OR ";
      }
    }
                  ...
                  <div class="modal-body">
                    <div class="mb-3">
                      ...
                      <select name="wisata" id="wisata" class="form-select" aria-label="Default select example" required>
                        <option value="<?= $row['id_wisata'] ?>"><?= $row['judul'] ?></option>
                        ...
                        foreach ($wisata as $rowsata) : ?>
                          <option value="<?= $rowsata['id_wisata'] ?>"><?= $rowsata['judul'] ?></option>
                        <?php endforeach; ?>
                      </select>
                    </div>
                    <div class="mb-3">
...
if ($_SESSION['page-url'] == "tour") {
  if (isset($_GET['key']) && $_GET['key'] != "") {
    ...
    foreach ($keys as $no => $data) {
      $data = strtolower($data);
      $quer .= "tbl_wisata.judul LIKE '%$data%' OR tbl_kategori.nama_kategori LIKE '%$data%' OR tbl_lokasi.nama_lokasi LIKE '%$data%'";
      if ($no + 1 < count($keys)) {
        $quer .= " OR ";
      }
    }
